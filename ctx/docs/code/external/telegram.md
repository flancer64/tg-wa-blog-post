...
При активном аварийном ручном режиме через `--source-file` чтение ru-сообщения из Telegram не является частью текущего run-cycle.
...
адаптер возвращает найденные данные без дополнительной обработки и сохраняет тип сообщения.
...
- публикует photo, video и document сообщения в указанный канал;
- выбирает метод отправки по типу сообщения;
...
Поддерживаются `sendMessage`, `sendPhoto`, `sendVideo` и `sendDocument`.

Медиа-контент публикуется без преобразования media-части.
