...
- создаётся `TeqFw_Di_Config_NamespaceRegistry`;
- выполняется сборка namespace registry через auto-discovery metadata;
- собранный registry передаётся в конфигурацию контейнера;
- контейнер переводится в frozen-состояние до резолва прикладных зависимостей;
- запрещено использование `container.register` (допускается только в тестах).

Bootstrap не содержит логики namespace-конфигурации и не допускает hardcoded filesystem mapping.
Обнаружение namespace полностью делегируется `TeqFw_Di_Config_NamespaceRegistry`.
Ручная регистрация namespace roots в bootstrap запрещена.
