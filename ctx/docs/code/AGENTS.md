...
- `testing/` — нормативные документы test-зон и их composition root.
...
Test-зоны используют разную модель сборки:

- unit-тесты создают production-компоненты напрямую через конструкторную DI-инъекцию без отдельного bootstrap;
