...
#### Unit Composition Model

Unit-тесты не используют отдельный bootstrap и не собирают DI-контейнер целиком.

Unit-сценарий:

- статически импортирует production-класс только внутри `*.test.mjs`;
- создаёт экземпляр напрямую через конструктор;
- передаёт все зависимости явными моками или стабами;
- не использует production bootstrap;
- не выполняет побочных действий;
- не работает с реальной конфигурацией.

Каждый unit-тест создаёт собственный набор зависимостей и экземпляров.

#### DI-модель unit

В unit-тестах:

- production-код импортируется статически только внутри `*.test.mjs`;
- зависимости подаются напрямую через конструктор;
- все внешние зависимости подменяются локальными моками;
