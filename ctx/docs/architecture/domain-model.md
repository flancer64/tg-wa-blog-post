...
Агрегат создаётся при обнаружении нового сообщения в ru-канале или при аварийном ручном запуске с локальным исходным файлом и соответствует ровно одному запуску CLI.

## Идентичность агрегата

Первичным идентификатором агрегата является `ru_message_id`. В штатном режиме уникальность `ru_message_id` гарантируется в пределах ru-канала и используется как доменная идентичность агрегата. В аварийном ручном режиме `ru_message_id` формируется вне Telegram как отрицательное целое число формата `-YYYYMMDDHHMM`, что отделяет его от штатных Telegram-идентификаторов. Дополнительный внутренний идентификатор не вводится.
...
Связь между ru, en и es публикациями инкапсулирована внутри агрегата. Отдельные внешние ссылки или обратные зависимости не формируют дополнительной доменной структуры. В аварийном ручном режиме локальный файл не становится самостоятельной доменной сущностью и не сохраняется как отдельная связь агрегата.
