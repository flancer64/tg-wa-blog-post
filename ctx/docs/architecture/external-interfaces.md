...
Система взаимодействует с двумя внешними API и двумя внешними текстовыми источниками:

- Telegram Bot API — получение и публикация сообщений;
- LLM API — языковое преобразование;
- локальный source-file артефакт — операторски заданный plain text источник ru-публикации для аварийного ручного режима;
...
- публикация photo, video и document сообщений в en-канал;
- публикация photo, video и document сообщений в es-канал.

Редактирование, удаление сообщений, обработка подписки на события и генерация media-контента не входят в архитектурную модель.

### Модель входных данных

Получаемое сообщение рассматривается как структурированная публикация и включает:

- `message_id`;
- `type` (`text | photo | video | document`);
- `text` как основной текст для сообщений типа `text`;
- `caption` как текст для сообщений с медиа;
- media-метаданные для сообщений с медиа;
- эксплуатационные атрибуты публикации.

В аварийном ручном режиме чтение из Telegram Bot API для ru-источника не выполняется.

---

## Интерфейс локального source-file

### Назначение

Локальный source-file используется как аварийный операторский источник ru-текста, когда получение сообщения из Telegram Bot API невозможно или было пропущено.

### Допустимые операции

В рамках архитектурной модели допускаются:

- указание пути к файлу через CLI-параметр `--source-file`;
- чтение plain text содержимого файла целиком как тела одной ru-публикации.

Запись в source-file, пакетная обработка нескольких файлов и чтение структурированных форматов не входят в архитектурную модель.

### Модель входных данных

Source-file содержит только plain text тело одной ru-публикации. Метаданные Telegram, внутренние поля агрегата и дополнительные служебные секции не входят в поддерживаемую модель.
...
Внешние интерфейсы Telegram Translation Publisher ограничены чтением и публикацией структурированных сообщений через Telegram Bot API, текстовыми преобразованиями через LLM API и использованием внешних prompt-артефактов. Взаимодействие описывается на уровне декларативных контрактов «сообщение → сообщение» и «текст → текст» без расширения функциональной области системы.
